...
class SalesHomePage(BasePage):
    ...
    用法:
        ...
        cart_page = sales_home.search_product("10004188").add_to_cart().open_cart()
    """
    ...
    ADD_TO_CART_ALT = [
        "span.linear_btn:has-text('加入购物车')",
        ".el-table__body span.linear_btn:has-text('加入购物车')",
        "text=加入购物车",
    ...
    def search_product(self, keyword: str) -> "SalesHomePage":
        ...
        # 等待搜索输入框可见
        self.wait_for_any_visible(*self.SEARCH_INPUT_ALT, timeout=5000)
        # 输入搜索关键字
    ...
    def add_to_cart(self, index: int = 1) -> "SalesHomePage":
        ...
        self.wait_for_any_visible(*self.ADD_TO_CART_ALT, timeout=5000)

        # 优先使用 span.linear_btn 精确定位
        target = f"span.linear_btn:has-text('加入购物车')"
        try:
    ...
    def open_cart(self) -> "CartPage":
        ...
        Returns:
            CartPage: 购物车页面对象（基于 popup 窗口的 page）
        """
        from pages.cart_page import CartPage
        ...
        return CartPage(cart_page_obj)

    def assert_search_results_visible(self) -> "SalesHomePage":
        """断言搜索结果显示（确认表格中至少有一个加入购物车按钮可见）"""
        logger.info("断言搜索结果可见")
        # 用 span.linear_btn 精确匹配，避免 text= 匹配到隐藏元素
        expect(self.locate("span.linear_btn:has-text('加入购物车')").first).to_be_visible(timeout=5000)
        return self
