...
### 盲等优化 reduce-blind-waits（2026-07-15，已归档）
- **变更名**：`optimize-test-waits`，Schema：spec-driven，归档于 `openspec/changes/archive/2026-07-15-optimize-test-waits/`
- **目标**：将约 36s 硬编码盲等替换为条件等待，test_sales_order 从 87s → 49s（-43%）
- **涉及文件**：`tests/test_sales_order.py`、`pages/cart_page.py`、`pages/order_confirm_page.py`、`pages/quotation_page.py`、`pages/customer_login_page.py`、`pages/enterprise_zone_page.py`、`tests/test_customer_allProducts.py`
- **主 Spec**：`openspec/specs/test-execution-optimization/spec.md`（4 个需求：盲等替换、冗余删除、复选框渲染保障、自测失败快速终止）
- **关键教训**：`test_sales_order` L282 `dialog.wait_for(state="hidden")` 初次实现有误——"添加产品"弹框的"确认"按钮只选中产品，不会关闭弹框。正确流程：确认 → 等 500ms → 点击关闭按钮 → `wait_for(state="hidden")`
- **关闭按钮定位**：必须用 `dialog.locator("i.el-icon-close.close")` 限定弹框范围，不能用 `page.locator()` 全页查找
- **新增导入**：`from playwright.sync_api import expect` 支持 `to_be_checked()` 验证
- **注意**：5 个回归验证任务（5.1-5.5）未完成即归档
